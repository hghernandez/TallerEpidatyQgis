...
provincias <- defu_2021 %>%
  ungroup()%>%
  distinct(IDPROV,NOMPROV)



all <- tidyr::crossing("IDPROV"=defu_2021$IDPROV,
                       "EDAD"=defu_2021$EDAD,
                       "SEXO"=defu_2021$SEXO,
                       "CAUSA"=defu_2021$CAUSA) %>%
  left_join(defu_2021 %>% ungroup() %>% select(-c("NOMPROV")),by= c("IDPROV","EDAD","SEXO","CAUSA")) %>%
